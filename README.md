...
> **Note:** This repo is private. If `git clone` fails with an auth error, install GitHub CLI first and authenticate:
>
> ```bash
...
> ```
>
> Then re-run the clone and bootstrap above. `bootstrap.sh` is idempotent — safe to run multiple times.
